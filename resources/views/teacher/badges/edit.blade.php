...
@section('teacher')
    ...
    <form method="POST" action="{{ route('teacher.badges.update', $badge) }}" style="max-width: 700px;" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="badge-image-section" style="margin-bottom: 1.5rem;">
            <input type="hidden" name="image_path" id="image_path" value="{{ $currentImagePath }}">
            ...
            <div id="badge_ai_panel" class="badge-panel" style="display: none;">
                ...
                <div id="badge_image_preview" class="badge-result-preview" style="margin-top: 1rem; {{ $currentImagePath ? '' : 'display: none;' }}">
                    <img id="badge_image_img" src="{{ $currentImagePath ? asset('storage/'.$currentImagePath) : '' }}" alt="" style="max-width: 120px; max-height: 120px; object-fit: contain; border-radius: 12px; border: 2px solid var(--eco-primary); {{ $currentImagePath ? '' : 'opacity: 0;' }} transition: opacity 0.4s ease;">
                </div>
